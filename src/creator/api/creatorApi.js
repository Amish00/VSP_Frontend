...
export const creatorApi = {
  ...
  // ─── UPDATED METHODS with `days` parameter ──────────────────
   getTopVideos: (limit = 5) =>
    axiosInstance.get(`/api/creator/analytics/top-videos?limit=${limit}`),

  // Other methods already accept days
  getViewsOverTime: (days = 180) =>
    axiosInstance.get(`/api/creator/analytics/views-over-time?days=${days}`),
  getContentBreakdown: (days = 30) =>
    axiosInstance.get(`/api/creator/analytics/content-breakdown?days=${days}`),
  getAnalyticsSummary: (days) =>
    axiosInstance.get(`/api/creator/analytics/summary?days=${days}`),
...
export const adminRevenueApi = {
  getMonthlyRevenue: (months = 12) =>
    axiosInstance.get('/api/admin/revenue/monthly', { params: { months } }),
  ...
  rejectPayout: (id, reason) =>
    axiosInstance.post(`/api/admin/revenue/payouts/${id}/reject`, { reason }),
  getPaymentRecords: (page = 0, size = 20) =>
    axiosInstance.get(`/api/admin/revenue/records?page=${page}&size=${size}`),
