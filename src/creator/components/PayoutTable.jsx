// PayoutTable.jsx (shared between creator and admin)
            ...
            <table className="w-full text-sm" style={{ minWidth: isAdmin ? 800 : 480 }}>
                <thead>
                    <tr className="border-b border-border bg-bg-el">
                        {columns.map(h => <th key={h} className="px-4 py-3 text-left text-xs font-bold text-text-muted uppercase tracking-wider">{h}</th>)}
                    </tr>
                </thead>
                <tbody>
                    {rows.map(row => (
                        <tr key={row.id} className="border-b border-border/50 last:border-0">
                            {isAdmin ? (
                                <>
                                    ...
                                    <td className="px-4 py-3"><Badge text={row.status} type={row.status === 'PROCESSED' ? 'approved' : row.status === 'REJECTED' ? 'rejected' : 'pending'} /></td>
                                    <td className="px-4 py-3">
                                        {row.status === 'PENDING' && (
                                            <div className="flex gap-2">
                                                <button onClick={() => onProcess?.(row.id)} className="px-2.5 py-1 rounded-lg bg-success/10 text-success text-xs font-semibold hover:bg-success/20">Process</button>
                                                <button onClick={() => onReject?.(row.id)} className="px-2.5 py-1 rounded-lg bg-danger/10 text-danger text-xs font-semibold hover:bg-danger/20">Reject</button>
                                            </div>
                                        )}
                                ...
                                <>
                                    ...
                                    <td className="px-4 py-3 font-semibold text-text-primary">${row.amount}</td>
                                    <td className="px-4 py-3 text-text-secondary">{row.withdrawalMethod}</td>
                                    <td className="px-4 py-3"><Badge text={row.status} type={row.status === 'PROCESSED' ? 'approved' : 'pending'} /></td>
