...
function Sec({ title, children, open: init = true }) {
  ...
  return (
    <div style={{ borderBottom: `1px solid ${videoTheme.border}` }}>
      <button
        ...
        style={{
          ...
          color: videoTheme.textMuted, fontSize: 9, fontWeight: 600,
...
function Row({ label, children }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 6 }}>
      <span style={{ fontSize: 10, color: videoTheme.textSecondary, minWidth: 48, flexShrink: 0 }}>{label}</span>
...
const RightPanel = () => {
  ...
  return (
    <div style={{
      ...
      borderLeft: `1px solid ${videoTheme.border}`,
      display: 'flex',
      flexDirection: 'column',
      overflow: 'hidden',
      flexShrink: 0,
    }}>
      <div style={{ padding: '9px 12px 8px', borderBottom: `1px solid ${videoTheme.border}`, flexShrink: 0 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 5 }}>
          <span style={{ fontSize: 12, fontWeight: 600, color: videoTheme.text, textTransform: 'capitalize' }}>{clip.type}</span>
        </div>
        <input
          value={clip.name || ''}
        ...
        <Sec title="Timing">
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 4 }}>
            {[['Start', 'start'], ['Dur.', 'duration'], ['Speed', 'speed'], ['Offset', 'offset']].map(([l, k]) => (
              <div key={k}>
                <div style={{ fontSize: 8, color: videoTheme.textMuted, marginBottom: 2, fontWeight: 600 }}>{l.toUpperCase()}</div>
          ...
          <Sec title="Position">
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 4 }}>
              {[['X %', 'overlayX'], ['Y %', 'overlayY'], ['W %', 'overlayW'], ['H %', 'overlayH']].map(([l, k]) => (
                <div key={k}>
                  <div style={{ fontSize: 8, color: videoTheme.textMuted, marginBottom: 2, fontWeight: 600 }}>{l}</div>
        ...
        {clip.type === 'text' && (
          <Sec title="Text">
            <div style={{ marginBottom: 7 }}>
              <div style={{ fontSize: 8, color: videoTheme.textMuted, marginBottom: 3, fontWeight: 600 }}>CONTENT</div>
              <textarea
                value={clip.text || ''}
            ...
            <Row label="Font">
              <select
                ...
                style={{ ...inp, padding: '3px 5px', fontSize: 10 }}
              >
            ...
            <Row label="Color">
              <input
                ...
                style={{ width: 24, height: 20, border: 'none', cursor: 'pointer', borderRadius: 4, padding: 0 }}
              />
              <input
                type="text"
                value={clip.textColor || ''}
                onChange={e => { if (/^#[0-9a-f]{6}$/i.test(e.target.value)) upd({ textColor: e.target.value }); }}
                ...
                {['left', 'center', 'right'].map(a => (
                  <button
                    ...
                    style={{
                      ...
                      height: 22,
                      borderRadius: 4,
                      border: `1px solid ${clip.textAlign === a ? videoTheme.borderLight : videoTheme.border}`,
                      background: clip.textAlign === a ? videoTheme.hov : videoTheme.el,
                      color: clip.textAlign === a ? videoTheme.text : videoTheme.textMuted,
                      cursor: 'pointer',
                      fontSize: 10,
                    }}
                  >
        ...
        {clip.type === 'shape' && (
          <Sec title="Shape">
            <Row label="Color">
              <input
                type="color"
                value={clip.shapeColor || '#3b82f6'}
                onChange={e => upd({ shapeColor: e.target.value })}
                style={{ width: 24, height: 20, border: 'none', cursor: 'pointer', borderRadius: 4, padding: 0 }}
              />
              <input
                type="text"
                value={clip.shapeColor || ''}
                onChange={e => { if (/^#[0-9a-f]{6}$/i.test(e.target.value)) upd({ shapeColor: e.target.value }); }}
                style={{ ...inp, flex: 1, fontSize: 10, padding: '3px 6px' }}
              />
            </Row>
          </Sec>
                ...
                return (
                  <div
                    ...
                    style={{
                      ...
                      textAlign: 'center',
                      fontSize: 10,
                      color: active ? videoTheme.text : videoTheme.textSecondary,
                    }}
                  >
                    {e.name}
                  </div>
            ...
            {['In', 'Out'].map(dir => {
              ...
              return (
                <div key={dir} style={{ marginBottom: 10 }}>
                  <div style={{ fontSize: 9, color: videoTheme.textMuted, fontWeight: 600, marginBottom: 4 }}>{dir.toUpperCase()}</div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    <span style={{ fontSize: 9, color: videoTheme.textMuted, minWidth: 28 }}>Dur.</span>
                    <input
                      type="range"
                      min={0.1}
                      max={3}
                    ...
                    />
                    <span style={{ fontSize: 9, color: videoTheme.textMuted, fontFamily: 'monospace', minWidth: 22 }}>{(clip[dkey] ?? 0.5).toFixed(1)}s</span>
                  </div>
                </div>
              );
            })}
