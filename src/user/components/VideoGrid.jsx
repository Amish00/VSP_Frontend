...
const VideoGrid = ({videos=[],onWatch,cols='grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4',gap='gap-4',emptyIcon='🎬',emptyTitle='No videos found'}) => {
  if(!videos.length) return <EmptyState icon={emptyIcon} title={emptyTitle}/>
  return(
    <div className={`grid ${cols} ${gap}`} role="list">
      {videos.map(v=><div key={v.id} role="listitem"><VideoCard video={v} onClick={onWatch}/></div>)}
    </div>
  )
}
