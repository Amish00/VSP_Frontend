...
const WatchPage = () => {
  ...
  // Keep a stable reference to showError
  ...
  // Fetch video – dependencies do NOT include showError
  useEffect(() => {
    ...
    if (id) fetchVideo();
  }, [id, user]); // <-- showError is NOT in deps

  // View count sync interval
  useEffect(() => {
  ...
  if (isPaidBlocked) {
    return (
      <div className="max-w-[1760px] mx-auto px-4 sm:px-6 lg:px-12 pt-[68px] md:pt-[92px] pb-[96px] md:pb-16">
          ...
          <div>
            <div className="relative rounded-xl overflow-hidden bg-black/80" style={{ aspectRatio: '16/9' }}>
              <div className="absolute inset-0 flex flex-col items-center justify-center text-center p-6">
                ...
                <button onClick={() => setPaidModalOpen(true)} className="px-5 py-2.5 bg-primary text-white rounded-xl font-semibold hover:bg-primary-dark transition">Upgrade to Watch</button>
              </div>
            ...
            <div className="flex flex-wrap items-center gap-3 pb-4 mb-4 border-b border-border">
              ...
              <button onClick={() => setPaidModalOpen(true)} className="px-5 py-2 rounded-xl text-sm font-bold bg-primary text-white shadow-[0_2px_8px_rgba(37,99,235,.4)]">Subscribe</button>
              <div className="flex items-center gap-2 ml-auto flex-wrap">
                <button className="flex items-center gap-1.5 px-3 py-2 rounded-xl border border-border bg-bg-el text-text-secondary text-sm font-semibold"><Heart size={14} fill="none" /> {formatNumber(likesCount)}</button>
        ...
        <LockedModal isOpen={paidModalOpen} onClose={handleModalClose} videoTitle={video.title} mode="signin_and_plans" />
  ...
  return (
    <div className="max-w-[1760px] mx-auto px-4 sm:px-6 lg:px-12 pt-[68px] md:pt-[92px] pb-[96px] md:pb-16">
        ...
        <div>
          ...
          <div className="flex flex-wrap items-center gap-3 pb-4 mb-4 border-b border-border">
            ...
            <div className="flex items-center gap-2 ml-auto flex-wrap">
              ...
              <button onClick={() => setShareModalOpen(true)} className="flex items-center gap-1.5 px-3 py-2 rounded-xl border border-border bg-bg-el text-text-secondary text-sm font-semibold hover:bg-bg-hov transition-all">
      ...
      <LockedModal isOpen={paidModalOpen} onClose={handleModalClose} videoTitle={video.title} mode="signin_and_plans" />
      <LockedModal isOpen={signInModalOpen} onClose={handleModalClose} mode="signin" />
      <ShareModal isOpen={shareModalOpen} onClose={() => setShareModalOpen(false)} url={`${window.location.origin}/watch/${video.id}`} title={video.title} thumbnail={video.thumbnailUrl} />
    </div>
  );
};
